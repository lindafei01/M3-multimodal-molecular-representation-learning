...
class Dtis(nn.Module):
    def __init__(self, dim=256, layer_output=3, layer_coa=1,
                 nhead=8, dropout=0.1, co_attention=False):
    ...
    def forward(self, f1, f2):
        ...
        if 'encoder' == self.co_attention:
            f1, f2 = self.encoder_coa_layers(f1, f2)
        elif 'stack' == self.co_attention:
            for i in range(self.layer_coa):
                f1, f2 = self.stack_coa_layers[i](f1, f2)
        else:
            for i in range(self.layer_coa):
                f1, f2 = self.inter_coa_layers[i](f1, f2)
        ...
        cat_vector = self.W_out[self.layer_output-1](cat_vector)
        # interaction = self.W_interaction(cat_vector)

        return cat_vector

    def __call__(self, f1, f2, train=True):

        # inputs = data.x, data.edge_index, data.protein
        # correct_interaction = data.y

        f_fused = self.forward(f1, f2)
        return f_fused
