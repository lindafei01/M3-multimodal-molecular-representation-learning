...
class MoleculeDataset(Dataset):
    ...
    def weights(self) -> List[float]:
        ...
        return [d.weight for d in self.data]
    ...
    def set_weights(self, weights: List[float]):
        assert len(self.data) == len(weights)
        for i in range(len(self.data)):
            self.data[i].set_weight(weights[i])
...
def split_data(data: MoleculeDataset,
    ...
    if split_type == 'crossval':
        ...
        if args.cv:
            train_val = MoleculeDataset([data[i] for i in idx[:train_val_size]])
            test = MoleculeDataset([data[i] for i in idx[train_val_size:]])

            return MoleculeDataset(train_val), MoleculeDataset(test)
...
def get_unimol_data_config(task='classification',
                data_type='molecule',
                epochs=10,
                learning_rate=1e-4,
                batch_size=16,
    ...
    yamlhandler = YamlHandler(config_path)
