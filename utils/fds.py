...
class FDS(nn.Module):
    ...
    def __init__(self, feature_dim, bucket_num=50, bucket_start=-3, start_update=0, start_smooth=1,
                 kernel='gaussian', ks=15, sigma=2, momentum=0.9):
    ...
    def _get_bucket_idx(self, label):
        ...
        _, bins_edges = np.histogram(a=np.array([], dtype=np.float32), bins=self.bucket_num, range=(-2., 4.))
        if label == 4.:
    ...
    def update_running_stats(self, features, labels, epoch):
        ...
        #logging.info(f"buckets [{len(buckets)}] ")
        for bucket in np.unique(buckets):
            curr_feats = features[torch.tensor((buckets == bucket).astype(np.uint8))]
            #logging.info(f"bucket [{bucket}] ")
            #logging.info(f"curr_feats [{curr_feats.shape}] ")
            curr_num_sample = curr_feats.size(0)
            curr_mean = torch.mean(curr_feats, 0) # dim=0 按列求平均值，返回的形状是（1，列数）
            curr_var = torch.var(curr_feats, 0, unbiased=True if curr_feats.size(0) != 1 else False)
            #curr_var = torch.var(curr_feats, 0)
            #logging.info(f"curr_feats [{curr_var}] ")
            #logging.info(f"curr_feats [{curr_feats.size(0)}] ")
            ...
            #logging.info(f"self.running_var[bucket - self.bucket_start] [{self.running_var}] ")
    ...
    def smooth(self, features, labels, epoch):
        ...
        buckets = np.array([self._get_bucket_idx(label) for label in labels])
        for bucket in np.unique(buckets):
            #logging.info(f"self.running_var[bucket - self.bucket_start] [{self.running_var_last_epoch[bucket - self.bucket_start]}] ")
