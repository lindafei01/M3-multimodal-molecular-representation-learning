...
@torch.no_grad()
def test(args, logs_dir, model, scaler, test_idx_loader, seq_data, seq_mask, gnn_data, geo_data, device, unimol_data=None, agile_data=None):

    for fold in range(args.fold):
        model_path = os.path.join(logs_dir, f'model_{fold}.pth')
        ...
        for idx in test_idx_loader:
            ...
            unimol_data_batch, agile_data_batch = prepare_data(args, idx, seq_data, seq_mask, 
                                                               gnn_data, geo_data, device,
                                                               training=False, 
                                                               unimol_data=unimol_data, 
                                                               agile_data=agile_data,
            ...
            _y_true.append(targets)
            _y_pred.append(preds)
        _y_true = torch.cat(_y_true, dim=0).detach().cpu().numpy()
        _y_pred = torch.cat(_y_pred, dim=0).detach().cpu().numpy()
    ...
    y_pred /= args.fold
    if scaler is not None and args.task_type == 'reg':
        preds = torch.tensor(scaler.inverse_transform(y_pred).astype(np.float64))
...
def main(args):
    ...
    # data split
    assert args.cv == True
    train_val_data, test_data = split_data(data=datas, split_type=args.split_type, sizes=args.split_sizes,
                                                 seed=args.seed, args=args)
    train_val_idx = [data.idx for data in train_val_data]
    ...
    # train_val_idx_loader = DataLoader(train_val_idx, batch_size=args.batch_size)
    ...
    if args.task_type == 'reg':
        train_val_smiles, train_targets = train_val_data.smiles(), train_val_data.targets()
        scaler = StandardScaler().fit(train_targets)
        scaled_targets = scaler.transform(train_targets).tolist()
        train_val_data.set_targets(scaled_targets)
        #TODO： 把weight求出来
        if args.LDS:
            weights = weighted_mse_loss(scaled_targets)
            train_val_data.set_weights(weights)
        for (id, value) in zip(train_val_idx, scaled_targets):
            datas[id].set_targets(value)
        if args.LDS:
            for (id, value) in zip(train_val_idx, weights):
    ...
    model = Multi_modal(args, compound_encoder_config, device, unimol_model_params)
    ...
    schedule = NoamLR(optimizer=optimizer, warmup_epochs=[args.warmup_epochs], total_epochs=[args.epochs],
                      steps_per_epoch=len(train_val_idx)*(args.split_sizes[0]) / (args.split_sizes[0]+args.split_sizes[1]) // args.batch_size, 
                      init_lr=[args.init_lr], max_lr=[args.max_lr], final_lr=[args.final_lr],)
    ids = list(range(len(train_val_data)))

    torch.backends.cudnn.enabled = False
    logger.info('train model ...')

    splitter = Splitter(args.split_type, args.seed, args.fold)


    train_val_true = []
    train_val_pred = []
    for fold, (tr_idx, va_idx) in enumerate(splitter.split(train_val_idx)):
        np.random.shuffle(ids)
        train_idx = [train_val_idx[idx] for idx in tr_idx]
        train_sampler = RandomSampler(train_idx)
        train_idx_loader = DataLoader(train_idx, batch_size=args.batch_size, sampler=train_sampler)

        val_idx = [train_val_idx[idx] for idx in va_idx]
        val_sampler = BatchSampler(val_idx, batch_size=args.batch_size, drop_last=False)
        ...
        for epoch in range(args.epochs):
            ...
            train_all_loss, train_lab_loss, train_cl_loss, model = train(args, model, optimizer, schedule, train_idx_loader,
                                                                        ...
                                                                        epoch=epoch, training=True, unimol_data=unimol_data, agile_data=agile_data)
            
            logger.info("fold:" + str(fold) + ", epoch:" + str(epoch) + ", all_loss:" + str(train_all_loss) + ", lab_loss:" + str(
                train_lab_loss) + ", cl_loss:" + str(train_cl_loss))
            
            # val
            model.eval()
            val_result, val_all_loss, val_lab_loss, val_cl_loss, model, y_true, y_pred = val(args, model, scaler, val_sampler, 
                                                                                             seq_data, seq_mask, datas, 
                                                                                             data_3d, device, unimol_data=unimol_data, agile_data=agile_data)

            writer.add_scalars(main_tag=f'fold_{fold}_loss',
            ...
            writer.add_scalars(main_tag=f'fold_{fold}_result', tag_scalar_dict={'val_result': val_result}, global_step=int(epoch + 1))
            if best_result is None or (best_result < val_result and args.task_type == 'class') or \
                ...
                torch.save(info, os.path.join(logs_dir, f'model_{fold}.pth'))
                train_val_fold_true = y_true.squeeze()
                train_val_fold_pred = y_pred.squeeze()

                best_result = val_result
                logger.info("--min_val_loss:" + str(val_all_loss) + ", val_result:" + str(val_result))
            

            print(f"fold{fold} epoch{epoch} finishes!")
            torch.cuda.empty_cache()
        
        train_val_true.extend(train_val_fold_true.astype(float))
        train_val_pred.extend(train_val_fold_pred.astype(float))
        print(f"fold{fold} finishes!")
    
    if not os.path.exists(os.path.join(logs_file1, "result")): 
        os.makedirs(os.path.join(logs_file1, "result"))            
    with open(os.path.join(logs_file1, "result", "val_result.json"), "w") as file:
        json.dump({"train_val_true": train_val_true, "train_val_pred": train_val_pred}, file)

    model = Multi_modal(args, compound_encoder_config, device, unimol_model_params=unimol_model_params)
    result, y_true, y_pred = test(args, logs_dir, model, scaler, test_sampler, 
                                  seq_data, seq_mask, datas, data_3d, device,
                                  unimol_data=unimol_data, agile_data=agile_data)
    logger.info("*************result:" + str(result) + "**************\n")

    with open(os.path.join(logs_file1, "result", "test_result.json"), "w") as file:
        json.dump({"y_true": y_true.tolist(), "y_pred": y_pred.tolist()}, file)
